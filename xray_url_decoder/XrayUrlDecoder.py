...
from xray_url_decoder.IsValid import (
    isValid_tls,
    isValid_reality,
    isValid_userVless,
    isValid_vnextVless,
...
def convertVmessLinkToStandardLink(link):
    ...
    url = urlunparse(
        Components(
            scheme="vmess",
            netloc="{username}@{hostname}:{port}".format(
                username=data["id"], hostname=data["add"], port=data["port"]
            ),
...
class XrayUrlDecoder:
                    ...
                    header = {
                        ...
                        "request": {
                            ...
                            "path": [
                                (
                                    self.getQuery("path")
                                    if self.getQuery("path") is not None
                                    else "/"
                                )
                            ],
                            "headers": {
                                "Host": [self.getQuery("host")],
                                "User-Agent": [""],
        ...
        match self.security:
            case "tls":
                ...
                tlsSettings = TLSSettings(
                    self.getQuery("sni"), fingerprint=self.getQuery("fp"), alpn=alpn
                )
            ...
            case "reality":
                realitySettings = RealitySettings(
                    self.getQuery("sni"),
                    self.getQuery("pbk"),
                    fingerprint=self.getQuery("fp"),
                    spider_x=self.getQuery("spx"),
    ...
    def vmess_json(self) -> Vmess:
        user = UserVmess(
            self.url.username,
            alterId=self.getQuery("aid"),
            security=self.getQuery("scy"),
        )
    ...
    def is_equal_to_config(self, config_srt: str) -> bool:
        ...
        match self.url.scheme:
            case "vless":
                return (
                    config["settings"]["vnext"][0]["users"][0]["id"]
                    == self.url.username
                    and config["settings"]["vnext"][0]["port"] == self.url.port
                    and config["settings"]["vnext"][0]["address"] == self.url.hostname
                )
            case "vmess":
                return (
                    config["settings"]["vnext"][0]["users"][0]["id"]
            ...
            case "trojan":
                return (
                    config["settings"]["servers"][0]["password"] == self.url.username
                    and config["settings"]["servers"][0]["port"] == self.url.port
                    and config["settings"]["servers"][0]["address"] == self.url.hostname
                )
